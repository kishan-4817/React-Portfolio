...
const BlogPostCard = ({ post }) => {
  return (
    <motion.article
      whileHover={{ y: -5 }}
      className="group relative bg-[var(--secondary)]/20 backdrop-blur-sm rounded-xl overflow-hidden border border-[var(--secondary)]/30 hover:border-[var(--primary)]/30 transition-all duration-300"
    >
      {/* Image Container */}
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between text-sm text-[var(--foreground)]/60">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="px-4 py-2 bg-[var(--primary)] text-[var(--foreground)] rounded-lg hover:opacity-90 transition-opacity"
          >
            Read More
          </motion.button>
        </div>
